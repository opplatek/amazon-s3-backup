...
main_dir="/home/mns/workspace/projects/ribothrypsis/samples"
bucket="s3://ribothrypsis-samples-manolis"
...
bckp=(
"hsa.dRNASeq.CEPH1463.polyA.1"
"hsa.dRNASeq.HeLa.total.REL3.long.1"
"hsa.PARE.Hela.siGL2_siXRN1.1"
"hsa.PARE.Hela.siGL2_siXRN1.2"
"hsa.PARE.Hela.siSMG6_siXRN1.1"
"hsa.PARE.Hela.siSMG6_siXRN1.2"
)
	...
	if tar -tzf $sync_dir/${dir}.tar.gz >/dev/null; then # Check if tar.gz is not corrupted
		...
		if `grep -w ${dir}.tar.gz $manifest | grep -q -w $filesize`; then # Check if we have seen an exact file name with and exact file size
			echo "Warning: ${dir}.tar.gz has been already uploaded before (same file_name and file_size); not going to sync."
	                ...
	                printf "%s\t%s\t%s\t%s\t%s\t%b\n" "$date" "$main_dir/$dir" "${dir}.tar.gz" "$filesize" "$md5sum" "$bucket" >> $manifest
